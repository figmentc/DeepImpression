...
def decompress(images):
    im = []
    for image in images:
        im.append(image.reshape(128, 128, 3))
    return np.array(im)


inputs = np.load("Train_Images.npy")
...
for monkey in range(2):
    print show_weights.shape
    print show_weights.T[monkey].shape
    plt.clf()
    plt.imshow(show_weights.T[monkey].reshape(128, 128, 3), cmap=plt.cm.gray)
    plt.draw()
    plt.show()
